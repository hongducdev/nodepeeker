...
- **Inspect Code Viewer (Default: Pure CSS):** Displays formatted code with full syntax highlighting (properties, hex colors with live color chips, values, units, comments, and line numbers). Toggle to **Tailwind CSS** with color-coded utility chips, or to **SVG** for the layer's exported markup with element/attribute highlighting and color swatches. The SVG markup is **fetched on demand**: it is not extracted with the selection, so the first time you open the SVG tab expect a brief pause while the layer is exported, after which the markup is cached for that layer. The same markup is **previewed above the code**: the exported SVG renders in a fixed-height, checkerboard-backed board — scaled down to fit, never up — so the artwork is visible without reading the markup, transparency shows through as the checkerboard, and white or dark artwork stays legible in both the light and dark themes. The preview reuses that same cached export, so opening the tab fetches nothing extra; while the markup is still loading, or if the export failed, no preview appears and the code area states the state. Switch tabs with **`1`/`C`** (CSS), **`2`/`T`** (Tailwind), or **`3`/`S`** (SVG), and press **`Ctrl + C`** / **`Cmd + C`** to copy the active tab.
...
- **Animation Export (MP4 / GIF):** Click **Download MP4** or **Download GIF** to encode the selection as an animation — useful for demos, loading states, and handing motion work to developers. Five things decide whether it works:
  - **A GIF layer exports as its original file, byte for byte.** Select an animated GIF and the plugin reads the GIF data straight from the layer's fill and hands it back unchanged — no re-encode, no quality loss, and **no plan size limit**, because Figma's encoder is never involved. The panel says *direct asset export* when this path applies.
  - **A video or Motion animation is encoded by Figma, which only accepts a frame placed directly on the page.** Figma encodes that **whole frame, not the layer you selected**, so the panel names the frame it will encode before you click — and a layer nested inside a page frame is labelled with that frame rather than the layer, because the layer itself cannot be encoded. **The section is active only when the plugin detects video, GIF, or animation in the selection** (a video fill, a GIF layer, a Motion timeline, applied animation styles, keyframes, or manual keyframe tracks); anything static renders the section blurred and unclickable, so there is never a button that fails. A frame nested inside a **Section**, or a layer outside any frame, is not a page-level frame Figma can encode either.
  - **Exports are automatically scaled down to fit your plan.** A free/starter plan rejects video above **1920 × 1080**; a page frame is routinely far taller (a landing page can be 21,000 px). The plugin clamps the height to fit automatically, so you get the export instead of Figma's *"exceeds the … limit for this plan"* error.
